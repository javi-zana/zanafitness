...
function MemberPanel({ member, onClose }: { member: Member; onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative w-full max-w-sm bg-[#141414] border-l border-[#242424] h-full overflow-y-auto flex flex-col z-10">
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#242424] sticky top-0 bg-[#141414]">
        ...
        <div className="p-5 space-y-5">
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              <div key={s.label} className="bg-[#141414] border border-[#242424] rounded p-3 text-center">
          ...
          <div className="bg-[#141414] border border-[#242424] rounded p-4">
          ...
          <div className="space-y-2 pt-1">
            ...
            <button
              onClick={onClose}
              className="w-full font-mono text-[8px] tracking-[0.25em] uppercase py-3 border border-[#242424] text-gray-400 rounded hover:text-white transition-colors"
            >
              Close
            </button>
          </div>
...
function Modal({ title, onClose, children }: { title: string; onClose: () => void; children: React.ReactNode }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-[#141414] border border-[#242424] rounded z-10">
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          <button
            key={s.label}
            onClick={s.onClick}
            className={`bg-[#141414] border border-[#242424] rounded p-5 text-left transition-colors ${s.onClick ? "hover:border-[#b3cdff]/30 cursor-pointer" : "cursor-default"}`}
          >
      ...
      <div className="bg-[#141414] border border-[#242424] rounded overflow-hidden">
        ...
        {MEMBERS.filter(m => !m.checkin).map((m, i, arr) => (
          <button
            ...
            className={`w-full flex items-center gap-3 px-5 py-3 text-left hover:bg-[#141414] transition-colors ${i < arr.length - 1 ? "border-b border-[#141414]" : ""}`}
          >
      ...
      <div className="bg-[#141414] border border-[#242424] rounded overflow-hidden">
        ...
        {INITIAL_POSTS.slice(0, 3).map((post, i) => (
          <div key={post.id} className={`flex items-start gap-3 px-5 py-4 ${i < 2 ? "border-b border-[#141414]" : ""}`}>
      ...
      {/* Next call CTA */}
      <div className="bg-[#141414] border border-[#86efac]/20 rounded p-5 flex items-center justify-between gap-4">
        <div>
...
function MembersTab({ coachEmail }: { coachEmail: string }) {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-5">
        ...
        <Modal title="Invite Member" onClose={() => { setShowInvite(false); setInviteStatus("idle"); setInviteError(""); }}>
          <div className="space-y-3">
            ...
            <button
              ...
              className="w-full font-mono text-[8px] tracking-[0.25em] uppercase py-3 bg-[#b3cdff] text-[#141414] rounded font-bold hover:bg-white transition-colors disabled:opacity-40"
            >
      ...
      <div className="bg-[#141414] border border-[#242424] rounded overflow-hidden">
        <div className="hidden sm:grid grid-cols-[1fr_100px_70px_90px_60px_36px] gap-3 px-5 py-3 border-b border-[#242424]">
        ...
        {filtered.map((m, i) => (
          <div key={m.id} className={`flex flex-col sm:grid sm:grid-cols-[1fr_100px_70px_90px_60px_36px] gap-2 sm:gap-3 px-5 py-4 items-start sm:items-center hover:bg-[#141414] transition-colors cursor-pointer ${i < filtered.length - 1 ? "border-b border-[#141414]" : ""}`}
            onClick={() => setSelectedMember(m)}>
        ...
        <div className="flex gap-2 mb-3 overflow-x-auto pb-1">
          {["Announcements", "General", "Check-ins", "Wins", "Q&A"].map(c => (
            <button key={c} onClick={() => setPostCategory(c)}
              className={`shrink-0 font-mono text-[7px] tracking-widest uppercase px-2.5 py-1.5 rounded border transition-colors ${postCategory === c ? "bg-[#b3cdff] text-[#141414] border-[#b3cdff]" : "text-gray-400 border-[#242424] hover:text-white"}`}
            >{c}</button>
          ))}
        </div>
        <textarea rows={3} placeholder="Share an announcement, insight, or reply to your members..."
          value={postContent} onChange={e => setPostContent(e.target.value)}
          className="w-full bg-[#141414] border border-[#242424] rounded px-4 py-3 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-[#b3cdff]/40 transition-colors resize-none leading-relaxed"
        />
        <div className="flex justify-end mt-3">
          <button onClick={submitPost} disabled={!postContent.trim()}
            className="font-mono text-[8px] tracking-[0.25em] uppercase px-5 py-2.5 bg-[#b3cdff] text-[#141414] rounded hover:bg-white transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
          >Post</button>
        </div>
      </div>
          ...
          {replyingTo === post.id && (
            <div className="mb-4 flex gap-2">
              <textarea rows={2} placeholder="Write your reply..."
                ...
                className="flex-1 bg-[#141414] border border-[#242424] rounded px-3 py-2 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-[#b3cdff]/40 resize-none"
              />
              <div className="flex flex-col gap-1">
                <button onClick={() => submitReply(post.id)} disabled={!replyText.trim()}
                  className="font-mono text-[8px] tracking-widest uppercase px-3 py-2 bg-[#b3cdff] text-[#141414] rounded hover:bg-white transition-colors disabled:opacity-30"
                >Send</button>
                <button onClick={() => { setReplyingTo(null); setReplyText(""); }}
                  className="font-mono text-[8px] tracking-widest uppercase px-3 py-2 border border-[#242424] text-gray-400 rounded hover:text-white transition-colors"
                >Cancel</button>
              </div>
            </div>
          )}
...
function ProgramsTab({ coachId }: { coachId: string }) {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-5">
      ...
      {programs.map(prog => (
        <div key={prog.id} className="bg-[#141414] border border-[#242424] rounded overflow-hidden">
          ...
          {expanded === prog.id && (
            <div className="border-t border-[#242424] px-5 pt-4 pb-5 space-y-3">
              {loadingId === prog.id ? (
                ...
                <div className="space-y-3">
                  {(contents[prog.id] ?? []).map(block => {
                    ...
                    return (
                      <div key={block.id} className="bg-[#141414] border border-[#242424] rounded p-4">
              ...
              {/* Add content form */}
              {addingTo === prog.id ? (
                <div className="space-y-3 bg-[#141414] border border-[#b3cdff]/20 rounded p-4">
                  ...
                  <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                    {(Object.keys(CONTENT_META) as ContentType[]).map(t => (
                      <button key={t} type="button" onClick={() => setNewType(t)}
                        className={`font-mono text-[8px] tracking-wide uppercase py-2 rounded border transition-colors ${newType === t ? "text-[#141414] border-transparent font-bold" : "text-gray-400 border-[#242424] hover:text-white"}`}
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => { setAddingTo(null); setNewTitle(""); setNewLabel(""); setNewBody(""); setSaveError(""); }}
                      className="flex-1 font-mono text-[8px] tracking-widest uppercase py-2.5 border border-[#242424] text-gray-400 rounded hover:text-white transition-colors">
                      Cancel
                    </button>
                    <button onClick={() => addContent(prog.id)} disabled={!newTitle.trim() || saving}
                      className="flex-1 font-mono text-[8px] tracking-widest uppercase py-2.5 bg-[#b3cdff] text-[#141414] rounded font-bold hover:bg-white transition-colors disabled:opacity-40">
                ...
                <button onClick={() => setAddingTo(prog.id)}
                  className="w-full font-mono text-[8px] tracking-widest uppercase py-3 border border-dashed border-[#242424] text-gray-500 rounded hover:border-[#b3cdff]/40 hover:text-[#b3cdff] transition-colors">
                  + Add Content Block
                </button>
              )}
            </div>
    ...
    <div className="max-w-2xl mx-auto space-y-5">
      {showAdd && (
        <Modal title="Add Event" onClose={() => setShowAdd(false)}>
          <div className="space-y-3">
            <input value={newTitle} onChange={e => setNewTitle(e.target.value)} placeholder="Event title" className="w-full bg-[#141414] border border-[#242424] rounded px-4 py-3 text-white placeholder-gray-600 focus:outline-none focus:border-[#b3cdff]/40 font-mono text-sm" />
            <input value={newDate} onChange={e => setNewDate(e.target.value)} placeholder="Date (e.g. Thu, May 22)" className="w-full bg-[#141414] border border-[#242424] rounded px-4 py-3 text-white placeholder-gray-600 focus:outline-none focus:border-[#b3cdff]/40 font-mono text-sm" />
            <input value={newTime} onChange={e => setNewTime(e.target.value)} placeholder="Time (e.g. 7:00 PM PHT)" className="w-full bg-[#141414] border border-[#242424] rounded px-4 py-3 text-white placeholder-gray-600 focus:outline-none focus:border-[#b3cdff]/40 font-mono text-sm" />
            <input value={newLink} onChange={e => setNewLink(e.target.value)} placeholder="Meeting link (optional)" className="w-full bg-[#141414] border border-[#242424] rounded px-4 py-3 text-white placeholder-gray-600 focus:outline-none focus:border-[#b3cdff]/40 font-mono text-sm" />
            <div className="flex gap-2">
              {["LIVE","DEADLINE","1:1","PROGRAM"].map(t => (
                <button key={t} onClick={() => setNewTag(t)} className={`flex-1 font-mono text-[8px] tracking-widest uppercase py-2 rounded border transition-colors ${newTag === t ? "bg-[#b3cdff] text-[#141414] border-[#b3cdff]" : "text-gray-400 border-[#242424] hover:text-white"}`}>{t}</button>
              ))}
            </div>
            <button onClick={addEvent} disabled={!newTitle.trim() || !newDate.trim()} className="w-full font-mono text-[8px] tracking-[0.25em] uppercase py-3 bg-[#b3cdff] text-[#141414] rounded hover:bg-white transition-colors disabled:opacity-30">Add Event</button>
          </div>
        </Modal>
      )}
      ...
      <div className="space-y-3">
        {events.map(event => (
          <div key={event.id} className="bg-[#141414] border border-[#242424] rounded p-5 flex items-center gap-4">
            ...
            <div className="w-px self-stretch bg-[#242424]" />
            <div className="flex-1 min-w-0">
            ...
            <div className="flex gap-2 shrink-0">
              ...
              <button
                ...
                className="font-mono text-[7px] tracking-widest uppercase px-2.5 py-1.5 border border-[#242424] text-gray-500 rounded hover:text-[#f87171] hover:border-[#f87171]/30 transition-colors"
              >Remove</button>
            </div>
          </div>
        ))}
...
function CoachMessagesTab({ userId, userName, userInitials, avatarColor }: {
  ...
  useEffect(() => {
    ...
    channelRef.current = supabase.channel(`coach-dm-${activeId}`)
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "direct_messages", filter: `conversation_id=eq.${activeId}` },
        (payload) => {
          const incoming = payload.new as CoachDM;
          setMessages(prev => prev.some(m => m.id === incoming.id) ? prev : [...prev, incoming]);
          setConvs(prev => prev.map(c => c.id === activeId ? { ...c, last_preview: incoming.content } : c));
        })
      .subscribe();
  ...
  async function sendMessage() {
    ...
    setInput("");
    await supabase.from("direct_messages").insert({ conversation_id: activeId, sender_id: userId, content: text });
  ...
  return (
    <div className="max-w-4xl mx-auto h-[calc(100dvh-130px)] md:h-[calc(100dvh-80px)] flex border border-[#242424] rounded overflow-hidden relative">
      {/* Conversation list */}
      <div className={`w-full md:w-72 shrink-0 bg-[#0a0f16] border-r border-[#242424] flex flex-col ${activeId ? "hidden md:flex" : "flex"}`}>
        <div className="px-4 py-3.5 border-b border-[#242424] flex items-center justify-between">
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {convs.map(conv => (
            <button key={conv.id} onClick={() => setActiveId(conv.id)}
              className={`w-full flex items-center gap-3 px-4 py-3.5 text-left transition-colors border-b border-[#1a222c] ${activeId === conv.id ? "bg-[#b3cdff]/5 border-l-2 border-l-[#b3cdff]" : "hover:bg-[#141414]"}`}>
      ...
      <div className={`flex-1 flex flex-col min-w-0 ${!activeId ? "hidden md:flex" : "flex"}`}>
        {activeConv ? (
          <>
            <div className="px-4 py-3.5 border-b border-[#242424] flex items-center gap-3 bg-[#0a0f16]">
                ...
                return (
                  <div key={msg.id} className={`flex items-end gap-2 ${isMe ? "justify-end" : "justify-start"}`}>
                    ...
                    <div className={`max-w-[72%] md:max-w-sm flex flex-col ${isMe ? "items-end" : "items-start"}`}>
                      <div className={`px-4 py-2.5 rounded-2xl text-sm leading-relaxed ${isMe ? "bg-[#b3cdff] text-[#141414] rounded-br-sm" : "bg-[#111820] text-white rounded-bl-sm"}`}>{msg.content}</div>
            ...
            <div className="p-3 border-t border-[#242424] flex gap-2 bg-[#0a0f16]">
              <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === "Enter" && !e.shiftKey && sendMessage()}
                placeholder={`Message ${activeConv.other.name}...`}
                className="flex-1 bg-[#141414] border border-[#242424] rounded-full px-4 py-2.5 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-[#b3cdff]/50 transition-colors"
              />
              <button onClick={sendMessage} disabled={!input.trim()}
                className="w-10 h-10 rounded-full bg-[#b3cdff] text-[#141414] flex items-center justify-center hover:bg-white transition-colors disabled:opacity-40 shrink-0">
      ...
      {showPicker && (
        <div className="absolute inset-0 z-30 flex items-center justify-center px-6" style={{ background: "rgba(0,0,0,0.6)" }}>
          <div className="bg-[#141414] border border-[#242424] rounded-xl w-full max-w-sm shadow-2xl overflow-hidden">
            ...
            <div className="px-4 py-3 border-b border-[#242424]">
              <input autoFocus value={search} onChange={e => setSearch(e.target.value)} placeholder="Search members..."
                className="w-full bg-[#141414] border border-[#242424] rounded px-3 py-2.5 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-[#b3cdff]/50 transition-colors" />
            </div>
...
export default function CoachDashboard({ profile }: { profile: ProfileData }) {
  ...
  return (
    <div className="min-h-screen bg-[#0f0f0f] text-white flex">
      ...
      {/* Sidebar */}
      <aside className="hidden md:flex flex-col w-60 bg-[#0a0a0a] border-r border-[#242424] fixed h-full z-20">
        <div className="px-5 py-6 border-b border-[#242424]">
        ...
        <div className="p-5 border-t border-[#242424] space-y-4">
          <div className="flex items-center gap-3">
      ...
      <main className="flex-1 md:ml-60 flex flex-col min-h-screen">
        <header className="sticky top-0 z-10 bg-[#0f0f0f]/95 backdrop-blur-md border-b border-[#242424] px-5 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
      ...
      {/* Mobile bottom nav */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-20 bg-[#0a0a0a]/95 backdrop-blur-md border-t border-[#242424] flex">
        {NAV.map(item => {
          const active = activeTab === item.id;
          return (
            <button key={item.id} onClick={() => setActiveTab(item.id)}
