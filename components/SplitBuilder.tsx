...
function emptyDay(n: number): SplitDay { return { id: uid(), label: `Day ${n}`, muscleGroups: [], exercises: [emptyExercise()] } }
...
export function SplitBuilder({
  ...
  const startDays = initial?.days ?? [emptyDay(1)]
  ...
  function addDay() {
    const d = emptyDay(days.length + 1)
    setDays(prev => [...prev, d])
    setExpandedId(d.id)
  }
  ...
  function addCustomGroup(dayId: string) {
    const val = (customInputs[dayId] ?? '').trim()
    if (!val) return
    setDays(prev => prev.map(d => {
      if (d.id !== dayId) return d
    ...
    setCustomInputs(prev => ({ ...prev, [dayId]: '' }))
  }
    ...
    setDays(prev => prev.map(d => {
      ...
      return {
        ...
        exercises: d.exercises.map(e => {
          ...
          if (field === 'sets') return { ...e, sets: value === '' ? null : Number(value) }
  ...
  async function handleSave() {
    setSaving(true)
    await onSave({ type: 'structured_split', days })
    setSaved(true)
        ...
        return (
          <div key={day.id} className="rounded-2xl bg-[var(--c-card)] border border-[var(--c-border)] overflow-hidden">
            ...
            <div className="flex items-center gap-2 px-4 py-3">
              ...
              >
                <p className="text-sm font-semibold text-[var(--c-text)] truncate">{day.label || `Day ${idx + 1}`}</p>
                {!isExpanded && day.muscleGroups.length > 0 && (
                  <p className="text-[10px] text-[var(--c-text4)] font-mono mt-0.5 truncate">{day.muscleGroups.join(' · ')}</p>
                )}
              </button>
              ...
              <div className="border-t border-[var(--c-border)] px-4 py-4 space-y-4">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {PRESET_MUSCLE_GROUPS.map(group => {
                    ...
                    {/* Custom muscle groups (added by coach) */}
                    {day.muscleGroups.filter(g => !PRESET_MUSCLE_GROUPS.includes(g)).map(group => (
                      <button
                        key={group}
                        onClick={() => toggleMuscleGroup(day.id, group)}
                        className="px-2.5 py-1 rounded-lg text-[11px] font-mono bg-[#b0e455] text-[#0f1a0c] font-semibold transition"
                      >
                        {group}
                      </button>
                    ))}
                  ...
                  <div className="flex gap-2 mt-2">
                    <input
                      type="text"
                      value={customInputs[day.id] ?? ''}
                      onChange={e => setCustomInputs(prev => ({ ...prev, [day.id]: e.target.value }))}
                    ...
                    <button
                      ...
                      disabled={!(customInputs[day.id] ?? '').trim()}
