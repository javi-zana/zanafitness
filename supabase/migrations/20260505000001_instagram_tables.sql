-- Instagram DM inbox tables

create table if not exists ig_conversations (
  id text primary key,                    -- Instagram sender PSID / IGSID
  ig_username text,                       -- populated lazily via Graph API
...
create table if not exists ig_messages (
  id text primary key,                    -- Instagram message MID
...
-- Service role only — coaches read via server components, no client-side access needed
